  ...
  return (
    <Screen scroll>
      ...
      <Text variant="body" color="textSecondary" style={{ marginTop: spacing.sm }}>
        Enter the 6-digit code sent to <Text variant="body" style={{ fontWeight: '700' }}>{phone ?? ''}</Text>
      </Text>
      <Text variant="caption" color="textTertiary" style={{ marginTop: 4 }}>
        Demo tip: enter <Text variant="caption" color="primary">000000</Text>
      </Text>

      <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: spacing['2xl'], marginBottom: spacing.base }}>
        {digits.map((d, i) => (
          <TextInput
            key={i}
      ...
      {error ? (
        <Text variant="bodySm" color="danger" style={{ marginBottom: spacing.md }}>{error}</Text>
      ) : null}

      <View style={{ alignItems: 'center', marginBottom: spacing.xl }}>
        {cooldown > 0 ? (
          <Text variant="bodySm" color="textSecondary">Resend code in {cooldown}s</Text>
        ) : (
          <Text
            variant="bodySm"
            color="primary"
      ...
      <Button label="Verify" onPress={submit} loading={loading} disabled={!canSubmit} />
    </Screen>
  );
}
