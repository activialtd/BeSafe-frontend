...
export default function EmergencyContactsOnboarding() {
  ...
  console.log(contacts, "the contacts lostsr");
  return (
    <Screen scroll>
      <Header />
      ...
      >
        Add up to 5 loved ones. If you trigger SOS during a ride, they'll
        instantly receive your live location and driver details.
      </Text>
      ...
      {contacts.length < 5 && (
        ...
        >
          ...
          />
          <Input
            placeholder="Relationship (e.g. Mum, Brother)"
            value={relationship}
            onChangeText={setRelationship}
          />
          {error ? (
            <Text variant="bodySm" color="danger" style={{ marginBottom: 8 }}>
              {error}
            </Text>
          ) : null}
          <Button
      ...
      <View style={{ marginTop: spacing.lg, gap: spacing.md }}>
        <Button
          label={contacts.length > 0 ? "Continue" : "Skip for now"}
          onPress={proceed}
        />
        {contacts.length === 0 && (
          <View
          ...
          >
            ...
            <Text variant="bodySm" color="textSecondary" style={{ flex: 1 }}>
              We strongly recommend adding at least one contact. SOS still
              notifies authorities either way.
            </Text>
          </View>
        )}
      </View>
