...
const VEHICLE_TYPES: { id: VehicleType; label: string; icon: keyof typeof Ionicons.glyphMap; hint: string }[] = [
...
export default function RegisterVehicle() {
  ...
  const step2Valid = useMemo(
    () => brand.length >= 2 && model.length >= 1 && year.length === 4 && color.length >= 3 && plate.replace(/\s/g, '').length >= 5,
  ...
  const submit = async () => {
    ...
    try {
      ...
      router.replace({ pathname: '/(driver)/qr-code', params: { vehicleId: v.id } });
    } finally {
      setSaving(false);
    }
  };
    ...
    <Screen scroll>
      ...
      {step === 1 && (
        <MotiView from={{ opacity: 0, translateY: 8 }} animate={{ opacity: 1, translateY: 0 }}>
          <Text variant="h3" style={{ marginBottom: 8 }}>What kind of vehicle?</Text>
              ...
              return (
                <Pressable key={v.id} onPress={() => setType(v.id)}>
                  ...
                  >
                    ...
                    >
                      <Ionicons name={v.icon} size={22} color={active ? '#fff' : colors.text} />
                    </View>
                    <View style={{ flex: 1, marginLeft: 12 }}>
                      <Text variant="body" style={{ fontWeight: '700' }}>{v.label}</Text>
                      <Text variant="bodySm" color="textSecondary">{v.hint}</Text>
                    </View>
          ...
          <View style={{ marginTop: spacing.xl }}>
            <Button label="Continue" onPress={() => setStep(2)} disabled={!type} />
          </View>
        </MotiView>
      )}
      ...
      {step === 2 && (
        <MotiView from={{ opacity: 0, translateY: 8 }} animate={{ opacity: 1, translateY: 0 }}>
          <Text variant="h3" style={{ marginBottom: 8 }}>Vehicle details</Text>
          <Text variant="body" color="textSecondary" style={{ marginBottom: spacing.lg }}>
            This information will show up when riders verify your vehicle.
          </Text>

          <View style={{ gap: spacing.md }}>
            <Input label="Brand" placeholder="Toyota" value={brand} onChangeText={setBrand} />
            <Input label="Model" placeholder="Corolla" value={model} onChangeText={setModel} />
            <View style={{ flexDirection: 'row', gap: spacing.md }}>
              <View style={{ flex: 1 }}>
                <Input label="Year" placeholder="2018" value={year} onChangeText={setYear} keyboardType="number-pad" maxLength={4} />
              </View>
              <View style={{ flex: 1 }}>
                <Input label="Color" placeholder="Silver" value={color} onChangeText={setColor} />
              </View>
            </View>
            <Input
              label="Plate number"
          ...
          <View style={{ marginTop: spacing.xl }}>
            <Button label="Continue" onPress={() => setStep(3)} disabled={!step2Valid} />
          </View>
        </MotiView>
      )}
      ...
      {step === 3 && (
        <MotiView from={{ opacity: 0, translateY: 8 }} animate={{ opacity: 1, translateY: 0 }}>
          <Text variant="h3" style={{ marginBottom: 8 }}>Review</Text>
          <Text variant="body" color="textSecondary" style={{ marginBottom: spacing.lg }}>
          ...
          <Card
            ...
            elevated={false}
          >
            <Ionicons name="qr-code" size={20} color={colors.primary} style={{ marginRight: 10 }} />
            <Text variant="bodySm" style={{ flex: 1 }}>
              A unique QR code will be generated for you to print and display in your vehicle.
            </Text>
          </Card>

          <View style={{ marginTop: spacing.xl, gap: spacing.md }}>
            <Button label="Register vehicle & generate QR" onPress={submit} loading={saving} />
            <Button label="Edit details" variant="ghost" onPress={() => setStep(2)} />
          </View>
        </MotiView>
      )}
    </Screen>
