...
export default function ProfileScreen() {
  ...
  const rows: Row[] = [
    { icon: 'people', label: 'Emergency contacts', onPress: () => router.push('/emergency-contacts') },
    ...
    <Screen scroll contentContainerStyle={{ paddingBottom: 100 }}>
      ...
      <Card padding="lg" style={{ marginBottom: spacing.lg }}>
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          {user?.photoUrl ? (
            <Image source={{ uri: user.photoUrl }} style={{ width: 68, height: 68, borderRadius: 34 }} />
          ) : (
            <View
              style={{
            ...
            >
              <Text variant="h1" color="primary">{user?.fullName?.[0] ?? 'U'}</Text>
            </View>
          )}
          <View style={{ flex: 1, marginLeft: spacing.base }}>
            <Text variant="h3">{user?.fullName}</Text>
            <Text variant="bodySm" color="textSecondary">{user?.phone}</Text>
            <View style={{ flexDirection: 'row', marginTop: 6, gap: 6 }}>
              <Badge tone="success" size="sm" label="Verified" icon={<Ionicons name="checkmark-circle" size={12} color={colors.success} />} />
